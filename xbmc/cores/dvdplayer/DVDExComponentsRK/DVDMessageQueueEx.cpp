...
#include "DVDMessageQueueEx.h"
#include "utils/log.h"
#include "threads/SingleLock.h"
#include "DVDClock.h"
#include "utils/MathUtils.h"

CDVDMessageQueueEx::CDVDMessageQueueEx(const std::string &owner) : m_hEvent(true), m_owner(owner)
{
  m_iDataSize     = 0;
...
CDVDMessageQueueEx::~CDVDMessageQueueEx()
{
  // remove all remaining messages
  Flush(CDVDMsg::NONE);
}

void CDVDMessageQueueEx::Init()
{
  m_iDataSize     = 0;
  m_bAbortRequest = false;
  m_bEmptied      = true;
...
void CDVDMessageQueueEx::Flush(CDVDMsg::Message type)
{
...
MsgQueueReturnCode CDVDMessageQueueEx::Put(CDVDMsg* pMsg, int priority)
{
  CSingleLock lock(m_section);

  if (!m_bInitialized)
  {
    CLog::Log(LOGWARNING, "CDVDMessageQueueEx(%s)::Put MSGQ_NOT_INITIALIZED", m_owner.c_str());
    pMsg->Release();
    return MSGQ_NOT_INITIALIZED;
  }
  if (!pMsg)
  {
    CLog::Log(LOGFATAL, "CDVDMessageQueueEx(%s)::Put MSGQ_INVALID_MSG", m_owner.c_str());
...
MsgQueueReturnCode CDVDMessageQueueEx::Get(CDVDMsg** pMsg, unsigned int iTimeoutInMilliSeconds, int &priority)
{
  CSingleLock lock(m_section);
...
MsgQueueReturnCode CDVDMessageQueueEx::LockFirst(CDVDMsg** pMsg, unsigned int iTimeoutInMilliSeconds, int &priority)
{
  CSingleLock lock(m_section);
...
MsgQueueReturnCode CDVDMessageQueueEx::Pop()
{
  CSingleLock lock(m_section);
...
unsigned CDVDMessageQueueEx::GetPacketCount(CDVDMsg::Message type)
{
  CSingleLock lock(m_section);
...
void CDVDMessageQueueEx::WaitUntilEmpty()
{
    CLog::Log(LOGNOTICE, "CDVDMessageQueueEx(%s)::WaitUntilEmpty", m_owner.c_str());
...
int CDVDMessageQueueEx::GetLevel() const
{
  CSingleLock lock(m_section);
...
int CDVDMessageQueueEx::GetTimeSize() const
{
  CSingleLock lock(m_section);
...
bool CDVDMessageQueueEx::IsDataBased() const
{
